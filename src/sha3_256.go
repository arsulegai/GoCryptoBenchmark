...
func (s *Sha3_256) Name() string {
	return "Sha3_256"
}

func (s *Sha3_256) Register(parent *flags.Command) error {
	_, err := parent.AddCommand(s.Name(), "Performs benchmark for Sha3_256", "Computes Sha3_256 of random data and reports the result along with benchmark", s)
	if err != nil {
		return err
	}
	return nil
...
func (s *Sha3_256) Compute(data []byte) ([]byte, error) {
	if s.Args.Algorithm == CRYPTO_ALGORITHM {
		result_bytes := sha3.Sum256(data)
		return result_bytes[:], nil
	} else if s.Args.Algorithm == OPENSSL_ALGORITHM {
		result_bytes, err := openssl.SHA3_256(data)
...
func (s *Sha3_256) Run(child interface{}) error {
